...
module.exports = (app, db) => {
    ...
    app.get('/api/v2/character/:id', getCharacterV2ById)
    ...
    async function getCharacterBySlug(req, res) {
        try {
            ...
            // let slug = db.escape(req.params.slug);
            // let slug = slug(name, ' ')
            let results = await CharacterModel.getCharacterBySlug(slug);
            console.log('le resultat du slug : ', results);
            // affiche l'élément de l'url pathname : api/tintins/5
            //console.log(parseurl(req).pathname)
    ...
    //         console.log('resultat perso :', characterId)
    ...
    async function getCharacterV2ById(req, res) {
        console.log('getCharacterV2ById');
        try {
            const id = req.params.id;
            let characterId = await CharacterModel.getOneCharacter(id);
    ...
    async function postCharacter(req, res) {
        ...
        if (user[0].role) {
            ...
            // console.log(character);
            if (character.code) {
                res.json({ status: 500, msg: 'There was a problem !', result: character });
            }
            res.json({ status: 200, msg: `The character ${req.body.nom} ${req.body.prenom} has been recorded`, result: character });

        } else {
            res.json({ status: 500, msg: 'You have no right!' });
