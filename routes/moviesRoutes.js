...
module.exports = (app, db) => {
    ...
    async function getMovieSearch(req, res) {
        try {
            ...
            console.log('code result :', res.status);
            if (results.length == 0) {
                res.status(404).json({
                    status: 404,
                    msg: 'Results not found',
    ...
    async function postMovie(req, res) {
        ...
        if (user[0].role === 'admin') {
            ...
            console.log(movie);
            if (movie.code) {
                res.json({ status: 500, msg: 'There was a problem !', result: movie });
            } else {
                res.json({ status: 200, msg: `the movie ${req.body.title} has been recorded`, result: movie });
        }
